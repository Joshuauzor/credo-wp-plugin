<?php
  ...
  if( ! class_exists( 'Credo_WP_List_Table' ) ) {
    require_once( Credo_DIR_PATH . 'includes/wp-classes/class-wp-list-table.php' );
  }

  if ( ! class_exists( 'Credo_Payment_List' ) ) {

    /**
     * Payment List Class to add list payments made
     * via the payment buttons
     */
    class Credo_Payment_List extends Credo_WP_List_Table {
      ...
      public function __construct() {
        ...
        parent::__construct( array(
          'singular' => __( 'Payment List', 'credo-pay' ),
          'plural'   => __( 'Payment Lists', 'credo-pay' ),
          'ajax'     => false
        ) );
      ...
      public function no_items() {
        _e( 'No payments have been made yet.', 'credo-pay' );
      }
      ...
      public function column_tx_ref( $item ) {
        ...
        $title = '<strong>' . get_post_meta( $item->ID, '_credo_payment_tx_ref', true ) . '</strong>';
      ...
      public function column_amount( $item ) {
        $amount = get_post_meta( $item->ID, '_credo_payment_amount', true );
      ...
      public function column_default( $item, $column_name ) {
        ...
        switch ( $column_name ) {
          ...
          case 'fullname':
          case 'status':
            return get_post_meta( $item->ID, '_credo_payment_' . $column_name, true );
          case 'date':
      ...
      function get_columns() {
        ...
        $columns = array(
          ...
          'tx_ref'  => __( 'Transaction Ref', 'credo-pay' ),
          'customer' => __( 'Customer', 'credo-pay' ),
          'fullname' => __('Customer Fullname', 'credo-pay'),
          'amount'  => __( 'Amount (' . $admin_settings->get_option_value( 'currency' ) . ')', 'credo-pay' ),
          'status'  => __( 'Status', 'credo-pay' ),
          'date'    => __( 'Date', 'credo-pay' ),
        );
      ...
      public function add_to_menu() {
        ...
        $hook = add_submenu_page(
          'credo-payment-forms',
          __( 'Credo Transaction List', 'credo-pay' ),
          __( 'Transactions', 'credo-pay' ),
          'manage_options',
          'payment_list',
      ...
      public function payment_list_table() {
        ...
        require_once( Credo_DIR_PATH . 'views/payment-list-table.php' );
      ...
      public static function get_payments( $post_per_page = 20, $page_number = 1 ) {
        ...
        $args = array(
          ...
          'orderby'          => ! empty( sanitize_text_field($_REQUEST['orderby']) ) ? sanitize_text_field($_REQUEST['orderby'])  : 'date',
      ...
      /**
       * Gets the total payments made through Credo
       *
      ...
      public function add_payment_list_post_type() {
        ...
        $args = array(
          'label'               => __( 'Payment Lists', 'credo-pay' ),
          'description'         => __( 'Credo payment lists', 'credo-pay' ),
